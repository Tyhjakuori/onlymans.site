...
$dates = $conn->query("SELECT DATE_FORMAT(created_at, '%Y'), COUNT(1) as highs_created FROM highlights GROUP BY DATE_FORMAT(created_at, '%Y')");
...
while ($row = mysqli_fetch_assoc($dates)) {
    $data[$row["DATE_FORMAT(created_at, '%Y')"]] = $row["highs_created"];
}
